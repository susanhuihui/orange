...
func (c *UsermessageController) Post() {
	...
	// 留言成功刷新页面
	c.Redirect("http://"+models.OnlineUrl+"/orange/Teacher/TeacherMessage/"+strconv.Itoa(userMessage.PassiveUserId), 302)
}
...
// @Success 200 {int} models.Usermessage.Id
// @Failure 403 body is empty
// @router /AddUsermessageOther/ [post]
func (c *UsermessageController) AddUsermessageOther() {
	userMessage := models.Usermessage{}
	if err := c.ParseForm(&userMessage); nil != err {
		beego.Error("PARSE FORM: ", err.Error())
		c.Data["json"] = err.Error()
